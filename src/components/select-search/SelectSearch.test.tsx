...
jest.mock('@hooks/useOutsideClick', () => ({
    useOutsideClick: (callback: () => void): { current: HTMLDivElement } => {
        lastOutsideClickCallback = callback;
        const ref = { current: document.createElement('div') };
        return ref;
    },
    __getLastCallback: (): (() => void) | null => lastOutsideClickCallback,
}));
...
describe('SelectSearch', () => {
    ...
    it('selecciona una opción y llama onChangeOption con (option, name)', () => {
        ...
        render(
            <SelectSearch label="L" value="" options={options as any} onChangeOption={onChange} name="miSelect" placeholder="p" />
        );

        fireEvent.click(screen.getByText('p'));
        fireEvent.click(screen.getByText('Tres'));

        expect(onChange).toHaveBeenCalledWith({ id: '3', value: '3', label: 'Tres' }, 'miSelect');
    });

    it('abre con Enter y permite seleccionar con Enter', () => {
        const onChange = jest.fn();
        render(
            <SelectSearch label="L" value="" options={options as any} onChangeOption={onChange} name="sel" placeholder="elige" />
        );
        ...
        expect(onChange).toHaveBeenCalledWith({ id: '1', value: '1', label: 'Uno' }, 'sel');
    });

    it('muestra el valor seleccionado cuando se proporciona un value', () => {
        render(<SelectSearch label="Selección" value="2" options={options as any} onChangeOption={jest.fn()} name="seleccion" />);
    ...
    it('cierra el dropdown cuando se hace click fuera', async () => {
        render(<SelectSearch value="" options={options as any} onChangeOption={jest.fn()} placeholder="Click fuera" />);
        ...
        await waitFor(() => expect(screen.queryByText('Uno')).not.toBeInTheDocument());
    });

    it('muestra "No se encontraron resultados" cuando no hay coincidencias', async () => {
        const user = userEvent.setup();
        render(<SelectSearch value="" options={options as any} onChangeOption={jest.fn()} placeholder="Buscar" />);
    ...
    it('maneja correctamente un array de opciones vacío', () => {
        render(<SelectSearch value="" options={[]} onChangeOption={jest.fn()} placeholder="Sin opciones" />);
